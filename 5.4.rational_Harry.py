...
def change_name_in_folder_by_func(get_name_func , directory=None):
    # iterate over files in
    # that directory
    for filename in os.scandir(directory):
        if filename.is_file():
            new_name = get_name_func(filename.path)
...
change_name_in_folder_by_func( get_name_from_Potter, directory )